...
export function toMs(value: Duration | undefined, fallback = 0): number {
  ...
  // Unit strings use a non-negative decimal with an optional unit.
  const match = /^\s*(\d+(?:\.\d+)?)\s*(ms|s|m)?\s*$/.exec(String(value));
  const ms = match ? Number(match[1]) * unitMs(match[2]) : NaN;
  if (!Number.isFinite(ms)) throw new Error(`Invalid duration "${value}" (use e.g. 500, "500ms", "1.5s", "2m")`);
  return ms;
}
